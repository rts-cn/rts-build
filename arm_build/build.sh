...
export OUT=/data/rts
export FREESWITCH_PREFIX=$OUT/freeswitch
...
ModuleCheck 'freeswitch'
if [ $? -eq 0 ]; then
    cd /usr/src/freeswitch/libs/libvpx && if [[ ! -f Makefile || "$2" == '--force-build' ]]; then CC=$ARCH-gcc CXX=$ARCH-g++ AR=$ARCH-ar LD=$ARCH-g++ AS=$ARCH-as STRIP=$ARCH-strip NM=$ARCH-nm ./configure --enable-pic --disable-docs --disable-examples --disable-install-bins --disable-install-srcs --disable-unit-tests --target=armv7-linux-gcc;fi
	cd /usr/src/freeswitch && if [[ ! -f Makefile || "$2" == '--force-build' ]]; then ./bootstrap.sh && ./configure --host=$ARCH --prefix=$FREESWITCH_PREFIX --disable-signalwire; fi && make  && make install
fi